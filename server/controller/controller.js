...
exports.find=(req,res)=>{
    ...
    else
    {
        ...
        .catch(err=>{
          res.status(500).send({message:err.message || "error occured while retriving user imformation"})
        })
    }
...
exports.update=(req,res)=>{
    ...
    Userdb.findById(id,req.body).then(data=>{
        if(!data)
        {
            res.status(404).send({message:`Cannot updata user with ${id}.Maybe user not found`})
